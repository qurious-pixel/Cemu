...
sudo add-apt-repository -y "deb http://apt.llvm.org/$CODENAME/ llvm-toolchain-$CODENAME-16 main"
sudo apt update -qq
sudo apt install -y ccache ninja-build cmake libgtk-3-dev libsecret-1-dev libgcrypt20-dev libsystemd-dev freeglut3-dev clang-16 nasm libpugixml-dev libcurl4-openssl-dev libglm-dev rapidjson-dev libzstd-dev lld appstream
